<template>
  <div class="wrapper">
    <div style="height: 60px; line-height: 60px; font-size: 20px; padding-left: 50px; color: white;
      background-color: rgba(0,0,0,0.2)">水闸管理系统</div>
        ...
        if (valid) {  // 表单校验合法
          this.request.post("/user/login", this.user).then(res => {
            if(res.code === '200') {
              ...
              // localStorage.setItem("water",JSON.stringify(res.data.water))
              // 动态设置当前用户的路由
              // console.log("s")
              ...
              // })
              // .then(res => {
              //   localStorage.setItem("waterg",JSON.stringify(res.data))
              //   console.log("a")
              // })
              setRoutes()
              this.$router.push("/")
...
.wrapper {
  ...
  background: url("~@/assets/ivana-cajina-dQejX2ucPBs-unsplash.jpg");
